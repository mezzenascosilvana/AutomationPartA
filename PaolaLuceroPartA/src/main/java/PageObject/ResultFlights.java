...
public class ResultFlights {
	...
	public Boolean verifyAirlinesPanel() {
		...
		System.out.println("The number of the flights into the panel for " + "(" + elementAirlineName + ")" + " is: "
				+ temp + " the number of the flights found after to apply the filter is: "
				+ numberoftheflightContainer);
		...
		for (WebElement cell : childrenElements) {
			...
			for (int i = 0; i <= childrenElementsbySpan.size();) {
				if (temp.contains(String.valueOf(price))) {
					...
					System.out.println("NO MATCH : The price of the flight is €: " + temp
	...
	public void showResult() {
		...
		for (WebElement cell : childrenElements) {
			...
			for (int i = 0; i <= childrenElementsbySpan.size(); i++) {
				if (i == 0) {
					System.out.println(childrenElementsbySpan.get(i).getText());
				}
				if (i == 3) {
					System.out.println(childrenElementsbySpan.get(i).getText());
					System.out.println("------------------------------------------");
